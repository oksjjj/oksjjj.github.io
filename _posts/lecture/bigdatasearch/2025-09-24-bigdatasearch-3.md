...
- **기존 BM25F (단순 정규화)**  
  ...
  - 모든 zone에 대해 동일한 $b$ 값으로 문서 길이를 보정.  
  - zone별 특성이 무시되는 한계가 있음.  

---

- **최종 BM25F 식 (zone-specific b 적용)**  
  ...
  - $B_z$를 활용한 zone별 정규화가 적용된 $\tilde{tf_i}$를 사용.  
  - → zone마다 다른 길이와 중요도를 반영하여, 단순 BM25F보다 더 정밀한 검색 점수를 산출.  
...
- **non-textual feature의 예시**  
  - PageRank, Page Age, Page Type 등 텍스트 외적 요인들이 포함됨.  
  - 이는 질의와 무관하게(query independent) 문서의 신뢰도나 중요도를 반영할 수 있음.  

- **추가 결합 방식**  
  - 기본 BM25 점수에 비텍스트 요소를 더하여 최종 점수를 계산:  

  $$
  RSV^{BM25} + \log(pagerank)
  $$  

  - 예: PageRank는 웹 링크 구조에 기반한 문서 중요도를 나타내므로, 이를 로그 스케일로 반영하여 지나치게 큰 값이 되지 않도록 조정함.  

- **$V_j(f_j)$의 의미**  
  - 특정 feature $f_j$가 관련 문서($R=1$)에서 나타날 확률과  
    비관련 문서($R=0$)에서 나타날 확률의 비율을 로그로 취한 값.  
  - → 특정 feature가 관련성에 기여하는 정도를 정량적으로 반영.  

- **λ (람다) 파라미터의 역할**  
  - feature별 중요도를 조정하기 위한 scaling 계수.  
  - 여러 비텍스트 feature가 있을 때 균형 잡힌 영향을 주도록 함.  

- **정리**  
  - BM25F는 텍스트 기반 특징뿐 아니라 **구조적/메타데이터 기반 특징**도 함께 반영할 수 있도록 확장된 모델.  
