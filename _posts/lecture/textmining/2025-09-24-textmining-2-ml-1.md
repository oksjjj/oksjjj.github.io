...
| Iteration $t$ | Gradient $\nabla_{\mathbf{w}} TrainLoss(\mathbf{w})$ | Updated parameter $\mathbf{w}$ |
|---------------|------------------------------------------------------|--------------------------------|
| 1             | $\tfrac{1}{3}(2([0,0]\cdot[1,1]-1)[1,1] + 2([0,0]\cdot[1,2]-3)[1,2]$<br> $+ 2([0,0]\cdot[1,4]-3)[1,4]) = [-4.67,\,-12.67]$ | $[0.47,\,1.27]$ |
...
|---------------|------------------------------------------------------|--------------------------------|
| 1             | $\tfrac{1}{3}(2([0,0]\cdot[1,1]-1)[1,1] + 2([0,0]\cdot[1,2]-3)[1,2]$<br> $+ 2([0,0]\cdot[1,4]-3)[1,4]) = [-4.67,\,-12.67]$ | $[0.47,\,1.27]$ |
| 2             | $\tfrac{1}{3}(2([0.47,1.27]\cdot[1,1]-1)[1,1] + 2([0.47,1.27]\cdot[1,2]-3)[1,2]$<br> $+ 2([0.47,1.27]\cdot[1,4]-3)[1,4]) = [2.18,\,7.24]$ | $[0.25,\,0.54]$ |
...
| 2             | $\tfrac{1}{3}(2([0.47,1.27]\cdot[1,1]-1)[1,1] + 2([0.47,1.27]\cdot[1,2]-3)[1,2]$<br> $+ 2([0.47,1.27]\cdot[1,4]-3)[1,4]) = [2.18,\,7.24]$ | $[0.25,\,0.54]$ |
| $\vdots$      | $\vdots$                                             | $\vdots$                       |
| 200 (수렴)    | $\tfrac{1}{3}(2([1,0.57]\cdot[1,1]-1)[1,1] + 2([1,0.57]\cdot[1,2]-3)[1,2]$<br> $+ 2([1,0.57]\cdot[1,4]-3)[1,4]) = [0,\,0]$ | $[1,\,0.57]$ |
...
> 
> 을 만족하는 점들의 집합이다.  
> 즉, $\varphi(x)$ 가 $\mathbf{w}$ 와 내적했을 때 0이 되는 점들이 결정 경계이다.
>
> **2. 내적과 수직성의 관계**  
> 벡터 내적은  
> 
...
> 
> 로 정의된다.  
> 여기서 $y \in \lbrace +1, -1 \rbrace$ 이므로 부호는 올바른 분류 여부를 나타내고, 크기는 결정 경계에서 얼마나 떨어져 있는지를 반영한다.  
>
> **2. 결정 경계의 정의**  
> 결정 경계는  
> 
...
> $$  
> 
> 이 되고, $\mathbf{w}$ 가 단위 벡터일 때는 $\|\varphi(x)\|\cos\theta$ 와 같다.  
>
...
> 
> 라는 공식이 된다.  
> 즉, $\mathbf{w}$ 와 $\varphi(x)$ 의 내적이 클수록 점은 초평면에서 멀리 있고, 0 에 가까울수록 초평면에 더 가깝다.  
>
> **4. $\|\mathbf{w}\| = 1$ 인 경우**  
> $\|\mathbf{w}\| = 1$ 이면 위 식은  
> 
...
> 
> 로 단순해진다.  
> 따라서 마진 $(\mathbf{w}\cdot\varphi(x))y$ 는 곧 데이터 인스턴스 $\varphi(x)$ 에서 결정 경계까지의 거리이다.  
>
> **5. 마진과 거리의 관계**  
> 올바른 분류( $y$ 와 $\mathbf{w}\cdot\varphi(x)$ 의 부호가 같음 )라면 마진은 양수이며, 이는 초평면과의 거리로서 **얼마나 확신 있게 예측했는지**를 나타낸다.  
> 잘못된 분류(부호가 다름)라면 마진은 음수이며, 이는 초평면과의 거리로 **얼마나 크게 잘못했는지**를 나타낸다.    
...
> 이러한 그래프 모양 때문에 '힌지(hinge)'라는 이름이 붙었다.  
>
> **2. 힌지 손실이 의미하는 것**  
...
> 이때는 안쪽 항 $1-(\mathbf{w}\cdot\varphi(x))y$ 를 $\mathbf{w}$ 에 대해 미분한다.  
>  
> 그래서  
> 
> $$\nabla_{\mathbf{w}} Loss_{\text{hinge}}(x,y,\mathbf{w})=
> \begin{cases}
> -y\,\varphi(x), & (\mathbf{w}\cdot\varphi(x))y<1 \\
> [0,0], & (\mathbf{w}\cdot\varphi(x))y\ge 1
> \end{cases}$$  
>  
> **2. 데이터 인스턴스별 계산 과정**  
> (1) $(x,y)=([0,2],1)$  
...
>   지역 최소값을 빠져나와 **전역 최소값(global minima)** 에 도달할 가능성이 생긴다.  
>  
> 즉, SGD의 불안정해 보이는 진동이 단점처럼 보일 수 있지만,  
...
>   계산 효율성과 수렴 안정성 사이에서 균형을 맞출 수 있다.  
>  
> **장점**  
