...
>  
> → love, song, album 은 모든 문서에 등장하므로 IDF = 0 → 중요도 0  
>  
> **2. 각 단어의 $tf_{t,d}$ 계산**  
> - 예시:  
>   $$  
...
>   $d_2$의 벡터 값은 $d_1$의 벡터 값보다 정확히 2배가 된다.  
> - 이 때문에 문서의 **길이 차이(length difference)** 로 인해  
...
> - 그러나 내적(inner product)을 계산하면 $d_{100}$ 의 벡터는 $d_1$ 벡터의 모든 항이 **100배로 증가**한다.  
> - 그 결과, 문서의 길이가 길어질수록 실제 내용적 차이가 없음에도  
>   쿼리와의 유사도가 과대평가된다.  
> - 따라서 단순한 내적은 **문서 길이에 매우 민감하게 반응**한다는 문제가 있다.    
