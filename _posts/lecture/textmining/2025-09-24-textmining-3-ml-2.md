...
- 선형 모델의 경우, **가설 집합**은 특성 추출기(feature extractor) $\varphi(x)$의 선택에 의해 결정된다.  
...
- **가설 집합(Hypothesis class)**이란 모델이 학습 과정에서 선택할 수 있는 함수들의 집합을 의미한다.  
...
- 새로운 가설 집합(hypothesis class)은 이전 것을 포괄하면서도 개념적으로 더 유연하다.  
