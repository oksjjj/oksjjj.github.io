...
>   해당 위치의 **정답 단어를 확률로 예측**하도록 모델을 학습시킨다.  
>  
>   $$  
>   L_{MLM}  
>   =  
...
>   $$  
>  
> - 이 식은 “마스크된 위치들의 평균 음의 로그 우도(negative log-likelihood)”를 의미한다.  
> - 정답 단어에 대해 모델이 높은 확률을 줄수록  
>   $\log P(x_i \mid h_i^L)$ 이 커지고,  
>   따라서 $L_{MLM}$ 은 작아진다.  
>  
> - 즉, $L_{MLM}$ 을 최소화한다는 것은  
>   모델이 **문맥(context)** 을 더 잘 이해하여  
>   **마스크된 단어를 복원하도록 파라미터를 조정하는 과정**이다.    
...
> - Weight-tying(가중치 결합)은 **입력 임베딩(embedding) 행렬**과  
>   **출력층의 가중치(unembedding) 행렬**을 **하나로 공유하는 기법**이다.  
> - 일반적인 언어 모델에서는  
>   - 입력 단어를 벡터로 바꾸기 위한 임베딩 행렬 $\mathbf{E}$  
>   - 단어 분포를 예측하기 위한 출력 가중치 행렬 $\mathbf{W}$  
>   를 별도로 사용한다.  
> - 그러나 두 행렬은 모두 **어휘집 크기 $|V|$ × 임베딩 차원 $d$** 형태이므로  
>   둘을 따로 두면 **동일한 형태의 파라미터를 중복해서 학습**하게 된다.  
>  
>   $$  
>   \mathbf{u} = h_i^L \mathbf{E}^\top,  
>   \qquad  
>   \mathbf{y} = \text{softmax}(\mathbf{u})  
>   $$  
>  
> - Weight-tying은 $\mathbf{W} = \mathbf{E}^\top$ 로 설정하여  
>   동일한 행렬을 입력 임베딩과 출력층에서 **함께 사용**한다.  
> - 이를 통해  
>   - 불필요한 파라미터를 줄이고(parameter reduction)  
>   - 모델의 일반화 성능을 높이며  
>   - 단어 임베딩과 출력 확률 계산 간의 **의미적 일관성**을 확보한다.  
>  
