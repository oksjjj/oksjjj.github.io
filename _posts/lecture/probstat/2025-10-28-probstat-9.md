...
> **강의 내용**  
>
> 그래서 이제 화면 보시면서 얘기하시는데  
> 그러면 이제 생성 모델이 무엇이냐 이런 거를 먼저 좀 생각해 보면 좋을 것 같아요.  
>
> 그래서 이제 뭐 예를 들어서  
> 텍스트가 주어지고 거기에 대해서 어떤 shape이나 이미지를 생성하는  
> 어떤 그런 AI 모델을 생각을 해보면  
> 거기서 이제 제네리티브 모델, 생성하는 모델이기 때문에  
> 생성하는 생성자가 있어야 되잖아요.  
>
> 그래서 그 생성자를 제너레이터라고 부릅니다. 보통.  
> 제네리티브 모델이기 때문에.  
>
> 그래서 제네레이터에는 크게 두 가지 인풋이 있는데요.  
>
> 하나는 이제 컨디션 인풋.  
> 그래서 우리가 텍스트 이미지 생성이라고 한다면  
> 텍스트 청크들, 토큰의 시리즈,  
...
> output distribution, 이미지 distribution의  
> 여러 가지 이미지 surface를  
> 표현할 수 있게 해주는 거예요.  
>
> 그러니까 인풋을 Bird라고 했을 때  
...
> **강의 내용**  
> 
> 그러면 이제 이런 걸 생각을 해볼게요.  
> Bird인데, Bird의 어떤 Latent Information을 통해서  
> 새의 각각의 모양들을 표현할 수 있을 것인가.  
> 이런 것들이 이제 되게 핵심이라고 보는데요.  
>
> 랜덤 배리어블인데, 이 주사위가 이제 여러 개가 있는 게  
> 어떤 걸 표현을 하는 거냐면  
> 그 랜덤성의 하나하나의 Feature들이  
> 도대체 이 생성되는 이미지의 어떠한 것들을  
> 결정을 할 것이냐 라고 생각해 볼 수 있을 것 같아요.  
>
> 그래서 첫 번째 주사위는 색에 대한 것.  
> 두 번째 주사위는 각각의 컴포넌트들이 어떻게 결합되는지, 각도에 대한 것.  
> 그리고 새 사이즈에 대한 것.  
> 여러 가지들을 이제 좀 결정할 수 있는,  
> Statistical하게 결정하는 거겠죠.  
> 결정할 수 있는 이런 주사위, 랜덤 배리어블이.  
>
> 그리고 이렇게 표현한다는 것 자체가  
> 이게 뭘 표현하려고 하는 거냐면  
...
>
> 굉장히 좀 신기한 건데  
> Multivariate Gaussian Distribution에서는 Semantic이 없어요.  
> 그냥 뭔가 각각의 축들이  
> 어떤 컬러나 앵글을 표현하라고 설정하지 않았습니다.  
>
> 근데 우리가 생성 모델에서 학습을 하다 보면  
> 그게 이제 명확하게 첫 번째 Feature는 무조건 컬러다 이건 아니지만  
...
> High Dimensional 랜덤 배리어블에 다 이제 자연적으로,  
> 좀 자연스럽게 다 Association이 된다.  
>
> 그런 것들이 되게 재밌는 점이고  
...
> **강의 내용**  
> 
> 그래서 이제 Data Generator에는 크게 두 가지 방법이 있을 수가 있습니다.  
> 이제 Direct Approach라는 게 있습니다.  
> 여기 앞에서 말했었던 GAN 같은 거는  
> Direct Approach를 사용하는 되게 중요한 모델 중의 하나인데.  
>
> 어떤 뜻이냐면  
> Random Variable Input을 갖고 있을 때  
> 그 Random Variable Input을 갖고  
> Output에 대한 데이터를 생성을 직접적으로 해주는  
> 그런 모델들을 Direct Approach 모델들이라고 하고.  
>
> 그리고 이제 Indirect Approach 같은 경우는  
> 전형적으로 Diffusion 모델 같은 애들이  
> Indirect Approach인데.  
> Neural Network Input에 Random Variable을 그냥 넣어서  
> 뭔가 나오는 게 아니라  
> 뭔가 추가적인 파이프라인이 또 있습니다.  
>
> 또 그 파이프라인의 하나의 component로써  
> Neural Network가 작동을 하고.  
> 보통 Energy Function 이런 걸로 표현을 하는데  
> 혹은 Score Function 이런 걸로 표현을 하는데.  
> 그런 Score 혹은 Energy에 대해서  
...
> 
> 네 그래서 크게.  
> 네 크게 이제 데이터 생성을 하는 어떤 파이프라인.  
> 이렇게 한번 봐보면 될 것 같아요.  
>
> 그래서 이제 뭐냐  
> 사람 얼굴을 만약에 생성을 한다고 한다면  
> 왼쪽과 같은 사람 얼굴들을 굉장히 많이 모아놓은 이런 데이터셋에  
> learner가 존재를 해서  
> 또 옵티멀한 파라미터를 찾게 되고  
> 그 파라미터가 Neural Network,  
> 아까 Generator의 어떤 파라미터로써 작용을 하고.  
>
> 그거는 이제 트레이닝 과정 때 하는 거고  
> 샘플링 할 때는 이제 inference time,  
> 테스트 타임이라고도 얘기하는데  
> 샘플링 할 때는  
> Gaussian Random Variable을  
> Generator에다 주입을 했을 때  
> 뭔가 이제 우리가 원하는 이미지가 나오기를 원하는 거.  
>
> 그래서 보통 거의 많은 생성 모델들이  
> 트레이닝 타임이랑 테스트 타임이 이렇게 나와요.  
>
> 근데 우리가 앞에서 살짝 봤었던  
> 클래시피케이션 같은 문제들,  
> 클래시피케이션 모델 같은 경우는  
...
> **강의 내용**  
> 
> 네 그래서 그 이제 Indirect Approach.  
> 이런 것들은 잠깐 살펴보자면.  
> 데이터가 존재를 하고 러너가 존재해서  
> 이제 score function,  
> 아까 말씀드렸습니다.  
> 말씀드렸었던 energy, likely road 이런 개념들을  
> 이제 스코어링 해준다.  
> 뭐 score function이라고 이렇게 얘기할 수 있는데.  
>
> 학습자, 학습한 어떤 트레이닝 methodology가 존재해서  
> score function의 값을 높이는 형식으로,  
> 혹은 낮추는 형식으로  
> 간접적으로 이제 요 스코어링을 하게 되고.  
>
> 그리고 이렇게 스코어링 된,  
> 스코어링 최적으로 된 어떤 데이터의 위치들이  
> 사실 샘플링을 했을 때 생성되는 데이터라고 보면 될 것 같아요.  
>
> 여기 트레이닝이랑 샘플링 이렇게 나눠져 있었죠.  
> 트레이닝은 score function을,  
> score를 높이거나 낮추는 거죠.  
>
> 그리고 샘플링은  
> 그렇게 만들어진 어떤 러너의 파라미터를 가지고 샘플링,  
> MCMC나 뭐 이런 알고리즘을 통해서 샘플링하는 거.  
>
> 요거는 사실 지금 파이프라인 보면  
> 이해가 잘 안 되실 것 같은데요.  
> 뒤에 이제 normalizing flow랑 diffusion model 배우면  
> 이게 어떤 뜻인지 이제 완벽하게 이해를 하실 수 있을 것 같습니다.  
>
> 오늘 강의는 사실 여기에 초점을 맞춰주시지는 않습니다.  
> 요거 이제 강의를 계속 진행하다 보면 나올 것 같고.      
...
> **강의 내용**  
> 
> 그럼 이제 생성 모델의 목표는 무엇이냐 라고 한다면  
> 좀 여러분들이 헷갈릴 수도 있을 것 같은데  
> 좀 명확하게 해야 될 것 같아요.  
>
...
>
> 그래서 트레이닝 데이터에  
> 어떠한 특정 트레이닝 데이터에 어떤 디스트리뷰션이 생겼다고 했을 때  
> 그 디스트리뷰션과 굉장히 닮은 애를 만들어야 해요.  
> 그게 목적입니다.  
>
> 만약에 트레이닝 데이터 완벽하게 똑같은 애를 만든다고 가정을 하면  
> GPT 같은 것도 어떻게 보면 generative model,  
> generative AI 기술의 하나라고 볼 수 있는데  
> 생성할 때마다 데이터셋과 완벽하게 align된 데이터들이  
> 생성이 되겠죠.  
>
> 완전히 똑같은 트레이닝 데이터를 복원을 하는 게 목적이라고 한다면  
> 근데 아시다시피 새로운 가치를 extrapolation이 되는 게  
> 현대 generative AI 모델들의 되게 큰 특징점이거든요.  
>
> 새로운 정보가 창출돼요.  
> 뭐 많이 잘 창출되지는 않는데.  
> 근데 지식의 창출이냐  
> 그게 창출된 지식이 현실의 어떤 physical knowledge들과  
> 위반이 됐을 때 그걸 hallucination이라고 하죠.  
>
> 그 두 가지의 어떤 항상 줄다리기가 있긴 한데  
> 어쨌든 extrapolation 하기 위해서는  
> 트레이닝 데이터에 완벽하게 매칭을 하는 게 아니라  
> 리얼리스틱하고 world knowledge를 잘 반영할 수 있는  
> 그런 새로운 데이터를 만드는 거.  
> 그런 것들이 생성 모델의 목표라고 보시면 될 것 같아요.  
>
> 그래서 이제 뭐 그럼 이걸 어떻게 quantification을 하느냐.  
> 이게 이제 language 모델 같은 경우는  
> 어떻게 evaluation 하느냐, 어떻게 평가를 하느냐  
> 이런 거랑 되게 비슷한 느낌인데  
> 사실 똑같은 어떤 철학적인 그런 목적,  
> 철학적인 배경이 있다고 생각하면 될 것 같아요.  
>
> 이제 test data를 생각을 하는 거죠.  
> 그래서 test data에 대해서  
> 저기 보면 $\log p_\theta$라고 얘기하는데  
> 이게 이제 앞에서 배웠었던 log-likelihood에서  
> 그 log-likelihood의 개념에서 나왔었던 notation이죠.  
>
> $\log p_\theta$가,  
> $p_\theta$는 여기서 statistical model이 되는 거고.  
>
> 그래서 이제 트레이닝을 훈련한 다음에  
> test data에다가 우리가 만들어 놓은 statistical model에 대한  
> 어떤 likelihood estimation을 했을 때  
> 얼마나 많은 에러가 생기냐.  
>
> 이게 generalization 에러가 얼마나 생기냐인데  
> 이게 이제 크면 클수록  
> 아까 말씀드렸었던 extrapolation과 hallucination의  
> 어떤 그 사이에 있는 것,  
> 그런 것들을 이제 표현하는 거다  
> 그렇게 생각하시면 될 것 같고요.  
>
...
> 요즘엔 갑론을박이 있는 것 같습니다.  
>
> extrapolation에 대한 철학적 정의가  
> 어떻게 돼야 되는 것인가,  
> 새로운 정보를 창출이 어떻게 정의가 돼야 되는지  
> 아직도 말이 많은 것 같습니다.  
>
> 그래서 language model을 좀 평가를 하기 위한  
> 최적의 매트릭이 무엇이냐  
> 이런 것도 다 거기서  
> 그런 어떤 생각에서 오는 것 같고요.  
>
...
> 트레이닝 데이터가 이제 $N$개가 이렇게 주어진다고 했을 때.  
> probability density의 특징이,  
> sum 했을 때 $1$이라고 했었잖아요.  
...
> 
> 트레이닝 데이터가 이렇게 있을 때.  
> 이건 이제 discrete한 케이스라서 봉들이 생길 때인데.  
> 트레이닝 데이터의 $y$축에 해당하는 것들,  
> 그거 다 summation 했을 때 $1$이 돼야 되는  
> 그런 조건이 있는 것들을  
> 한번 생각해볼 수 있을 것 같아요.     
...
> 학습을 한 다음 실제로 density 이렇게 estimation을 해보면  
> 이런 모양이 많이 나옵니다.  
>
> 이런 모양이 잘 나오면  
> 전문가가 봤을 때 트레이닝 잘 됐다고  
> 판단하는 경우도 많습니다.  
...
> 이제 이거죠.  
> 이거.  
> 이 내용 보시고 꼭 처음 보시는 내용처럼 생각하시면 안 돼요.  
> 시험에도 나왔고, 앞에서 되게 많이 다뤘습니다.  
>
> 그래서 이제 maximum likelihood estimation,  
> $p_\theta^\*$,  
...
> 그래서 계산을 쭉 따라서  
> constant 항이 나와지고,  
> 그리고 그 $p_{\text{data}}$와 $p_\theta$의 어떤 관계들,  
> log 함수를 통한 어떤 decompose 이런 것들을 하다 보면  
> 맨 마지막에  
> $\arg\max \log p_\theta$를 하는 거가 되는 거고,  
>
> 그게 물리적으로 봤을 때  
> 위에서 그림으로 표현하는 것처럼  
> 데이터가 있는 데는 확률을 높이고  
...
> 그러니까 이제 알고리즘을 통해서  
> maximization을 하다 보면  
> 이게 예를 들어서 왼쪽에 어떤 회색의 영역이 있었는데  
> maximization을 취하다 보면  
> 이게 이제 optimization이라는 건  
> 한 번에 딱 되는 게 아니라  
> 시간을 걸쳐서 iterative하게 되는 거거든요.  
>
> 그래서 점진적으로  
> mass가 존재하는 데는 초록색을 통해서 확률이 높아지고  
...
> 여기까지가 개요구요.  
> 네.  
> 오늘의 교육은, 이게 강의 길이가 꽤 길어서  
> 아마 다음 주까지 연결해서 할 수도 있을 것 같습니다.  
> 코드 실습하면서.  
>
> 그래서 이제 variational 2-encoder에 대해서  
> 한 번 살펴보도록 하겠습니다.  
>
> 그래서 앞에 이제 제가  
> direct approach 중에 VAE가  
> direct approach에 가깝다고 얘기를 했던 데,  
> 먼저 이런 걸 한 번 생각을 해 볼게요.  
> 그림에서.  
>
> 그래서 $z$라는 latent variable,  
> 그러니까 gaussian random variable이라고 얘기를 했었고,  
> 그게 이제 아까 말씀드렸던 것처럼  
> size, color, breed 여러 가지 어떤 feature에 대한  
> 인간이 정의하지는 않았지만  
...
> 다 이렇게 구조를 갖고 있습니다.  
>
> pipeline 뜯어보고 모델 아키텍처들을 뜯어보면  
> 결국엔 이렇게 하는 거고,  
> 그래서 이렇게 좀 개념적으로 파악을 하셔야 될 것 같고.          
...
> **강의 내용**  
> 
> 그래서 그러면 이제 좀 distributional한 센스로 얘기를 해보면.  
> 이제 latent variable을 샘플링합니다.  
> $p(z)$라는 probability density로부터  
> $z$라는 random sample, latent variable이 있고  
> 지금 보면 distribution에서 distribution으로 가는 걸로  
> 이제 그림이 어떤 우리의 이야기가 격상이 됐죠.  
>
> 그래서 이게 파란색 distribution으로부터,  
> 파란색 distribution이 굉장히 간단합니다.  
> 정보가 없어요.  
>
> 거기서 이제 이..  
> 아까 요런 어떤 피규어에서 봤던 것들.  
> 요거는 1-dimensional한 데이터를 고려하는 건데  
> 실제 데이터는 굉장히 high-dimensional하잖아요.  
> 굉장히 high-dimensional하고  
> 굉장히 이런 density를  
> 이렇게 $z$축으로, 봉우리로 표현하기도 어려운데.  
>
> 이런 식으로 이제 오른쪽 굉장히 multi-modal한 density의...  
> 어떤 로컬 미니멈, 로컬 맥시멈들이 굉장히 많은 형태의  
> 굉장히 복잡한 곳으로  
> 맵핑을 해주는 게 제네레이터다.  
> 이렇게 생각하시면 될 것 같아요.  
>
> 그래서 이 앞에서 단순하게 우리가 버벌하게 정의를 했었던  
> 포즈, 사이즈 이런 것들이 사실 알고 보면  
> 우리가 수학적으로 생각을 했을 때는  
...
> 그래서 그럼 제네레이터가 무엇이냐.  
> 수학적으로 조금 더 생각을 해보면  
> 그러면 $p(z)$는 latent variable에 대한 distribution이라 하고  
> $p_\theta(x)$는 우리가 제네레이터가  
> $\theta$라는 파라미터로 parameterization 됐기 때문에  
> 그 파라미터를 지나서 그 결과물에 대한 distribution이기 때문에  
> $p_\theta(x)$로 표현했습니다.  
>
> 그럼 이제 제네레이터는  
> 우리 컨디셔널 probability를 배웠었는데  
> 컨디셔널한 probability로 제네레이터를  
> 확률적으로 생각을 할 수 있어요.  
>
> 그래서 우리가 이제 컨디셔닝  
...
>
> 그래서 이제 여기서부터 확률을  
> 우리가 앞에서 배웠었던 되게 재미없었던 도구들이  
> 얘를 다루기 위한 되게 중요한 도구들로  
> 탈바꿈을 합니다.  
>
> 마지널라이제이션 같은 경우는  
> 얘의 어떤 평균치를 어떻게 움직이느냐  
> 이런 게 되는 거고  
> 그리고 이제 레이턴트 배리어블이 하나인데  
> 레이턴트 배리어블이 여러 개 있을 때  
> 정보량들이 걔네들이 서로 어떻게 연결되고 하는 것들을  
> 다 공부를 했었습니다.  
> 앞에 수식 같은 걸로.  
>
> 그런 것들이 이제 뒤에서  
...
> **강의 내용**  
> 
> 네 그래서 우리가 KL Divergence이라는 거 공부를 했는데,  
> 그리고 Total variance도 공부를 했는데  
> KL Divergence를 우리가 메인으로 많이 쓴다고 했잖아요.  
> 제가 분명히 그때 당시에 얘기를 했던 게  
> 거의 모든 생성 모델은 다 KL Divergence와 커플링 되어 있다  
> 라고 얘기를 했고.  
>
> 그래서 여기서 나왔습니다.  
> 그 KL Divergence를 줄이는 게 우리의 목표인 거예요.  
>
> 근데 여기 줄이는 관점에서  
> 이게 데이터가 굉장히 크면  
> $\theta$가 아무리 크더라도  
> 이 KL Divergence를 일정 이하로만 줄이는 게 가능하지  
> 0으로 만들 수는 없거든요.  
> 데이터가 너무나 크고  
...
> 그러니까 미니마이제이션을 하지만  
> 이거 로컬 미니멈적인 미니마이제이션이고  
> 글로벌 미니마이제이션은 불가능하다고.  
> 뉴럴 네트워크, 논리니어한 머신을 다루고 있기 때문에.  
>
> 그러면 이제 KL Divergence가 어떤 의미였는지  
> 잘 생각을 해보면  
> 그게 두 분포의 어떤 다름을 정의하는  
> 그런 functional이었잖아요.  
>
> 그래서 이제 데이터에 대한 디스트리뷰션이 있고  
> 모델이 만들어낸 생성된 데이터들에 대한 디스트리뷰션  
> $p_\theta(x)$가 있을 때  
> 이 두 개를 최대한 같게끔,  
> 이 두 개의 어떤 다름을 최대한 줄이는  
> 그런 형태를 우리가 목적으로 하는 겁니다.  
>
> 그래서 만약에 우리가 만들어낸,  
> 만약에 이 KL Divergence가 정말 작은 값이라고 한다면  
> 우리가 어떤 걸 기대할 수 있냐면  
> 우리가 만들어낸 $x$에 대한 probability distribution  
> $p_\theta(x)$가 $p_{\text{data}}$와 되게 닮았으니까  
> 그러니까 생성된 데이터 이미지가,  
> 여기서부터 샘플링된 이미지가  
...
>
> 이게 이제 그 훈련.  
> 아까 앞에는 input과 output에 대한 철학이라고 한다면  
> 얘는 이제 훈련 방법론에 대한 철학입니다.  
>
> 그래서 우리는 classification은 어땠었죠?  
> classification은 하나의 이미지가 있고  
> 거기에 해당하는 레이블이 존재해서  
> 이 one to one 매칭을 하는 그런 느낌이라서  
> 걔네는 벡터와 벡터를 매칭을 하고  
> one to one을 매칭을 하는  
> one to one dependent한 objective function을 갖게 되는데  
> 여기서는 그렇지가 않습니다.  
>
> 여기서는 probability distribution을 매칭하는 거기 때문에  
> 다대다 매칭이에요.  
> 왜냐하면 $p_{\text{data}}$는 얼마나 많은 데이터가 있고  
> $p_\theta$를 만들 수 있는 데이터가 얼마나 많겠습니까?  
> 걔네들에 대해서 pairing이 없어요.  
> 디스트리뷰션으로 매칭을 하고.  
>
> 그래서 그게 철학적 다름입니다.  
> 그래서 우리가 KL Divergence 같은 걸 공부를 하는 거예요.  
> 다대다 매칭을 하기 위해서.  
>
> 그러니까 다대다 매칭을 한다는 게 어떤 의미냐면  
> 우리가 이제 다시 돌아가죠.  
> 첫 번째 이 슬라이드를 봤을 때  
> 이 랜덤 베리어블이  
> 랜덤 베리어블을 뽑을 때마다  
...
>
> 그렇습니다.  
> 그래서 우리는 이 KL Divergence와  
> maximum likelihood의 estimation을 연결을 했었고  
...
> 네.  
> 이제 그.  
> 조금 더 이제 디테일하게 한번 가봅시다.  
>
> 그래서 우리가 $\log p_\theta$가  
> 어떤 우리가 생성된 데이터에 대한 디스트리뷰션이라고  
> 얘기를 했었잖아요.  
>
> $\theta$는 다시 말해서 뉴럴 네트워크의 파라미터였고.  
>
> 그래서 이제 $p_\theta(x)$를  
> 이 마지널라이제이션,  
> 이 컨디셔닝하는 룰을 통해서,  
> 앞에서 배웠었던 그 룰을 통해 가지고  
> 중간에 이 적분 형식으로 쪼개 본다고 한번 해봅시다.  
>
> 이게 정확하게 equal이 성립을 하거든요.  
> 컨디셔널 프로비러빌리티의 정의에 따라 가지고.  
>
> 근데 여기서 $p(z)$는 Gaussian 랜덤 배리어블이고,  
> $p_\theta(x \mid z)$는 제네레이터였었죠.  
>
> 근데 그렇게 했을 때  
> $p_\theta(x)$,  
> 데이터의 생성에 대한 데이터 디스트리뷰션에 대한 게  
> 이렇게 연결되는 겁니다.  
>
> 우리가 앞에서 그냥 그린 파이프라인에서는  
> 데이터에 뭐,  
> 그러니까 모델에다가 데이터를 넣었다,  
> 랜덤 배리어블을 넣었다,  
> 레이턴트 배리어블을 넣었다,  
> 이런 뭐 넣었다 뺐다 이런 얘기를 했는데  
> 수학적으로 이제 명확하게 정의를 한 겁니다.  
>
> 그렇게 포멀하지 않은,  
> 인포멀한 거를 포멀한 단계로 바꾼 거예요.  
>
> 그래서 이 적분의 형태로 표현하는 게  
> 이제 이 제네레이터를 표현할 수 있는  
> 방법 중 하나다.  
>
> 근데 이제 여기서 문제는 뭐냐면,  
> 이제 그러면 $\theta$를  
...
> VAE라는.  
> 오늘 배우는.  
> 이 스트럭처의 모든 걸 다 표현한다고 생각하시면 될 것 같아요.  
>
> 이게 좀 어떻게 보면 어려울 수도 있는데.  
> 또 뭔가 하면 또 어렵진 않거든요.  
> 그래서 요거를 완벽하게 수학적으로 이해를 하면  
> VAE가 아 그래서 그런 수학적으로 모델링해서  
> 이런 의미가 되는거구나 라고 바로 깨달을 수가 있습니다.  
>
> 그래서 요런 식의 어떤 수식을 묘사를 하는 것을  
> 레이턴트 배리어블 모델이라고 합니다.  
> probability, applied statistics에서  
> 굉장히 많이 쓰이는 모델이고,  
> 사실 여러분들이 통계학으로 뭔가를 하시는 분들은  
> 굉장히 이런 걸 많이 하십니다.  
> 베이지안 통계 이런 거 하시는 분들.  
>
> 그래서 $\log p_\theta(x)$를  
...
> 하나씩 한번 살펴봅시다.  
>
> 그래서 첫 번째 $\log p_\theta(x)$가 있고  
> 이것을 맥시마이제이션 하는 것이  
> 우리의 목표였었죠.  
>
> 다음 줄을 보면  
> 어떤 $\int q(z)\,\log p_\theta(x)\,dz$라는 것으로 표현을 합니다.  
> 그런데 이게 우리가 적분을 했을 때,  
> 여기 보면 equality가 성립하는 이유는 뭐냐면  
> $\log p_\theta(x)$는 $z$와 관련이 없는 거잖아요.  
>
> 그래서 $\log p_\theta(x)$가 사실 밖으로 나오면  
> $q(z)$를 적분하는 겁니다.  
> 그런데 모든 probability distribution을 적분하면  
> 뭐가 됐었죠?  
> 1이거든요.  
>
> 그러니까 이것이 성립을 하는 거죠.  
...
>
> 그래서 $p_\theta(x)$를,  
> 이 괄호 안에 들어가 있는 게 다 똑같은 모양이에요.  
> 이것이 Bayes rule이라고 합니다.  
> 굉장히 중요한 철학이라  
> 여러분들 강의에서 더 디테일하게 다루진 않겠지만  
> 직접 찾아보시면 좋을 것 같아요.  
>
...
> 일단 먼저 해놓고,  
> 이제 이 밑으로 나누는 것은  
> log 함수의 특징에 따라서 나눠질 수 있어요.  
>
> 그래서 $\log ab$는  
> $\log a$ 더하기 $\log b$로 나눠지거든요.  
> 우리 고등학교 때 수학 시간 때  
> 생각을 해보면.  
>
> 그래서 이것을 이제 잘 조립을 해가지고  
> 표현을 하면 최종적인 식이 나옵니다.  
> 그래서 총 3개가 있는데  
> 이 3개를 neural network로 잘 표현하는 게  
> 그냥 VAE라고 보면 될 것 같아요.  
>
> 그래서 이제 하나씩 하나씩 한번 알아 봅시다.  
>
> 그래서 이제 우리는 intractable,  
> $p_\theta(x)$라는 것을  
> 우리가 직접적으로 수학적으로  
> 표현하는 것이 불가능해요.  
>
...
> **강의 내용**  
> 
> 그래서 첫 번째 우리가 $\log p_\theta(x)$를 이런 수식을 통해서  
> 유도를 해서 맨 마지막 항을 남겨놨는데.  
>
> 여기서 이제 tractable 하다는 것은  
> 우리가 뭔가 계산할 수 있다는 것이고  
> intractable 하다는 것은  
> 계산할 수 없다는 겁니다.  
>
> 그러면 계산을 할 수 없는데  
> 왜 도대체 이런 모델이 왜 필요하냐고 하면은,  
> 되게 컴퓨터 공학 하시는 분들은  
...
>
> 그래서 이제 세 번째 term을 control을 하지 않고  
> 그냥 가만히 이렇게 발산하게 놔두더라도  
> 앞에 두 개를 control해서  
> 우리가 최종적으로 $\log p_\theta(x)$를  
> control할 수 있다.  
> 이런 철학이 있습니다.  
>
> 그래서 intractable한  
...
> 이런 모양.  
>
> 그래서 이제 이것을 좌변 우변으로 정리한 거고.  
> 그래서 intractable 한 두 개를 왼쪽에 넣고.  
> tractable 한 것을 오른쪽에 넣고.  
>
> 그래서 이렇게 조합을 했을 때  
> 이렇게 표현되면  
> 이 inequality로만 표현할 수 있으면  
> 그걸 evidence lower bound라는  
> 굉장히 중요한 개념이 나오게 됩니다.  
>
> lower bound냐 upper bound냐  
> 이거 들어보신 적 있나요?  
>
> $f(x)$가  
> 마이너스 $x^2$ 플러스 $c$ 라는 함수가 있다고 하면은,  
> 얘의 upper bound는 $c$에요.  
>
> 어떤 뭔가가 변화할 때  
> 최대로 될 수 있는  
> 어떤 그 threshold를  
...
> 그냥 무시하고  
> 걔를 오른쪽으로 넘긴 다음  
> 어떤 양수의 무언가의 값을 계산을 하고  
> 이 equality를 inequality로 바꾸면  
>
> 그럼 이제 lower bound가  
> 명확하게 표현되는 것이기 때문에  
> 그런 것들을 evidence lower bound라고 하고  
>
> evidence라고 얘기하는 것은  
> 뭐냐면  
...
>
> 그래서 $q(z)$를  
> 이제 $q_{\pi}(z \mid x)$로 바꾸고,  
>
> 그리고 $p_{\theta}(z)$를  
> 그냥 이제 좀 prior knowledge가 가미된,  
> 이미 알고 있는 어떤  
> $p(z)$로 바꿀 수 있습니다.  
> 이거는 어떤 그냥 테크닉이라고 생각하시면 될 것 같고.
...
> **강의 내용**  
> 
> 그래서 어떻게 해서 잘 가공해서 테크닉을 써서 바꾸게 되면  
> 요런 ELBO,  
> 아까 ELBO가 변형된 동치꼴의 뭐가 나오는데,  
> 요걸 이제 해석하기 위해서 밑에 파이프라인을 보셔야 할 것 같아요.  
> 이것이 VAE의 파이프라인이라고 생각하시면 될 것 같습니다.  
>
> 그리고 뒤에 이제 $z$랑 디코더랑 $x'$이 표현되어 있는데  
> 여기서 이게 우리가 아까 앞에서 봤었던 파이프라인에서  
> 제네레이터로 표현되어 있는 파이프라인이고 그것이 반쪽인 것이고  
> VAE에서는 앞에 반쪽이 더 있습니다.  
> 되게 독특하게 생겼습니다.  
>
> 그래서 이것이 어떻게 동작하는지  
> 먼저 수학적인 것은 조금 뒤로 미루고 동작하는 원리에 대해서 한번 얘기를 해볼게요.  
>
> 그래서 이제 VAE는 훈련을 시키기 위해서 원래, 본래 $x$를 인코더라는 스트럭처를 통해서  
> 이 중간에 $q_{\phi}(z \mid x)$라고 표현되어 있잖아요.  
> 그 디스트리뷰션으로 매칭을 시켜 놓습니다.  
>
> 그래서 이것을 인코딩이라고 하고,  
> 인코딩, 디코딩 여러분이 들어보셨잖아요.  
> 인코딩하는 것은 정보를 줄이는 것이고 디코딩하는 것은 줄여진 정보를 다시 복원하는 것입니다.  
>
> 그래서 인코더, 디코더 그런 것들은 통신이나 그런 이론에서 굉장히 중요한 파운데이션인데  
...
> 거기서도 이제 ELBO를 논하니까.  
>
> 그래서 $x$라는 원래 오리지널 이미지를 인코더를 통해서  
> 중간에 $q_{\phi}$라는 어떤 우리가 미리 정해놓은  
> 그리고 뉴럴 네트워크로 파라메터라이징 할 수 있는 디스트리뷰션으로 매핑을 하고  
> 이제 뒤에서 보겠지만 이것이 가우시안 디스트리뷰션이 될 겁니다.  
>
> 그래서 거기서 이제 샘플을 합니다.  
> 샘플링을 합니다.  
> $z$라는 랜덤 배리어블을 샘플을 합니다.  
>
> 그런데 이 $z$가 이제 우리가 앞에서 파인프라인에서 봤었던  
> 원래 제네레이터의 그 $z$가 되는 거죠.  
>
> 그래서 여기서 이제 $p_{\theta}$를 $p$로 바꾼다는 게 그런 의미인 거고  
> 그래서 이렇게 표현이 되어 있어서  
> $z$라는 것을 디코더를 통해서 $x'$이라는 것으로 다시 변환을 한 다음에  
>
> 그러면 이제 $x$라는 원본 이미지가, 원본 데이터가  
> 인코더랑 디코더를 거치면서  
> 이런 arbitrary한 모양으로 바뀌게 되거든요.  
>
> 그래서 그거를 이제 그 다름을 좀 줄이기 위해서  
> $x$랑 $x'$이 이제 모양이 같도록 하는 reconstruction loss를 정의하게 됩니다.  
>
> 그래서 이제 그게 첫 번째 이제 VAE에 되게 중요한 어떤 파트로서  
> reconstruction loss,  
> $x$가 인코더를 거치고 디코더를 거쳐서 다시 reconstruct 됐을 때  
> 구조적 모양을 완벽하게 똑같이 따라야 된다.  
>
> 그게 이제 reconstruction loss로 정의가 됩니다.  
>
> 그래서 이 첫 번째.  
> 이 초록색으로 박스로 되어 있는 게 그런 어떤 rationale을 표현하는 그런 걸  
> 이제 초록색 박스로 표현해서 이게 reconstruction loss에 대한 거다.  
>
> 그래서 지금 수식을 잘 보면  
> $x$가 들어가서 $q_{\phi}$ 보면 $x$가 인풋이고 $z$가 아웃풋이잖아요.  
>
> 그래서 그게 이제 인코더에 대한 부분이었고  
> 그리고 그렇게 표현된 $z$가 다시 $\log p_{\theta}$에 인풋 $z$로 들어간 다음에  
> $x$로 다시 나왔잖아요.  
>
> 이 구조, 이 수학적 구조가 사실 이때 파이프라인이 설명하고 있는 거죠.  
>
> 그런 식으로 보시면 될 것 같고.    
...
> 어렵지 않은 부위를 다르게 이제 디스트리뷰션 웨이트를 주는 방식으로  
