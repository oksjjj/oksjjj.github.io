...
> 
> 우리가 $q_{\phi}$를 모델링 했는데,  
> Neural Network 혹은 파라미터로 모델링 하겠다고 했는데,  
> 
> 이 모양이 컨트롤되지 않으면  
> $z$를 샘플링 할 수가 없거든요.  
>
> $z$를 샘플링을 해야지만  
> 인코더, 디코더 스트럭처를 통해 표현할 수 있기 때문에  
> 
> $p(z)$는 Gaussian Distribution이나  
> 알 수 있는 Distribution을 고려하고,  
> KL divergence를 통해서  
> $q_{\phi}$와 $p$의 거리를 줄입니다.   
>
> 이게 최종적으로 0이 되면  
> $q_{\phi}$가 Gaussian Random Variable와 똑같은 것이 되고,  
> 그냥 Gaussian Random Variable에서 샘플링해서 쓰면 되잖아요.  
>
> 그래서 모양적으로 컨트롤하는  
> regularization loss,  
> 두 번째 loss가 있습니다.  
>
> 첫 번째 loss와 두 번째 loss가 포괄적으로 표현되어 학습을 하는 것.  
> 그것이 VAE의 전체적인 학습 파이프라인이라고 보시면 될 것 같아요.  
>
> 이게 결국에는 전부 다 여기서 온 거거든요.  
> latent variable에서.  
> 
> 이거 보면 VAE가 조금은 이해가 가실 것 같아요.  
> 좀 간접적인 레벨에서라도.  
>
> $\log p_{\theta}(x)$를 계산하기 위해서  
> 이상한 걸 많이 했는데  
> 이런 구조를 통해서 그걸 풀겠다는 게  
> VAE의 철학이라고 보시면 될 것 같아요.  
>
> Diffusion Model, Latent Variable Model, Normalizing Flow Model  
> 다 똑같은 얘기를 하는데  
> 완전히 다른 접근 방식을 가지고 갑니다.  
>
> 그래서 이 큰 파이프라인은 완벽하게 이해를 하셔야 될 것 같아요.  
> (수학적인 디테일은 모를 수 있지만)  
> $x$가 인코더로 들어가서 디코더로 표현되어서 복원이 되고  
>
> 이런 구조를 알아야 코드를 봤을 때  
> 아 이게 이 파트였구나  
> 이렇게 바로 이제 이해를 하실 것 같아요.  
>
> 이것은 앞으로 코딩을 직접 진행을 하시면  
> 이해를 하실 수 있는 부분이라고 생각을 합니다.  
...
> 제가 디테일한 건 얘기는 안 드리겠습니다만  
> 
> Reconstruction이라는 게  
> 결국에는 구조적으로 같게 만들어야 되는데  
>
> 이렇게 같게 만들기 위해서  
> 우리가 가장 많이 사용하는 메트릭 중 하나가  
> L2 Loss 유클리디언 디스턴스거든요.  
> 
> Negative Log-likehood를 계산을 하다 보면  
>
> 결국에는 Input으로 들어가는 $x$와 Output으로 나오는 $x'$ 사이에  
> 유클리디언 디스턴스를 취하고  
> $\sigma_0$를 통해서 스케일을 조절하는 이런 형태로 나옵니다.  
> 
> Reconstruction Loss는 다른 것이 아니라  
> Input $x$와,  
> 뭔가 안에서 찰흙처럼 빚어진 $x'$에 대한 거리다.  
> 
> 이게 만약에 0이 되면 모양이 완전히 똑같겠죠.  
> 그것이 Reconstruction Loss의 형태입니다.  
> 
> 이렇게 Reconstruction loss를 해 놓고  
> 그 다음에 Regularization을 하는 거죠.
...
> **강의 내용**  
> 
> Regularization도 디테일은 조금 생략을 하고  
> 
> Neural Network가 $q_{\phi}$에 $\mu$와 $\sigma$를 만들어 내게 됩니다.  
>
> $q_{\phi}$가 Gaussian Distribution인데,  
> 파라미터들이 Neural Network로부터 생성됐다고 가정을 하고  
> 
> $p(z)$는 아까 말씀드렸던 것처럼  
> Standard Gaussian Distribution이라고 해서  
> 
> KL Divergence를 서로 다른 Gaussian Distribution끼리 계산을 하면  
> explicit한 솔루션이 나오게 됩니다  
> 
> 실제로 계산을 해 보면 L2랑 비슷한 형태가 나오는데  
> 
> 이런 것도 앞으로 실제 코드에서 볼 수 있습니다
...
> **강의 내용**  
> 
> 지금 보면 Neural Network가 두 개가 있죠.  
> $\theta$라는 Neural Network가 있고,  
> $\phi$라는 Neural Network가 있었습니다.  
>   
> $\theta$는 디코더로서  
> $z$라는 Random Variable로부터  
> 원본 데이터를 복원하는 Neural Network이고  
> 
> $\phi$라는 Neural Network는  
> $p(z)$를 Approximation 하기 위해서,  
> Gaussian Parameter를 만들어 내는  
> Neural Network입니다.  
>   
...
> VAE의 치명적인 단점이 있는데요.  
> 
> Neural Network가 두 개잖아요.  
> 왜 두 개지?  
> 라고 물어보실 수도 있을 것 같아요.  
>   
> 우리가 비즈니스를 하기 위해서  
> 엄청나게 큰 시스템을 만들어 놓는 데 있어서  
>
> 모델이 하나에서 두 개가 되는 것은  
> Complexity가 두 배가 된다는 건데  
> 그러면 인프라도 두 배가 필요한 거거든요  
> 
> 똑같은 양의 데이터를 처리하기 위해서  
> 인프라가 두 배가 있어야 한다.  
> 말이 안 되는 거예요 사실.  
> 
> VAE를 현대 생성 모델에서 잘 안 쓰는 이유가 그런 겁니다.  
> 학습하는 파라미터가 너무 많이 필요해요.  
> 
> 두 개의 Neural Network가  
> 동시다발적으로 학습이 되어야 되기 때문에.  
> 
> 심지어 그렇게 학습하더라도 데이터 퀄리티가 잘 나오는 것도 아니고  
> 
> Diffusion 모델 등 선진화된 모델들이 데이터의 퀄리티가 더 좋고요.  
>
> 그런 단점이 거시적인 관점에서 봤을 때 있다는 거고  
>   
> 이렇게 훈련을 할 때  
> 두 개의 네트워크를 이런 식으로 학습하는 건 알겠는데  
> 
> (5 페이지)  
> 아까 앞에서 모든 생성 모델들은 크게 두 가지를 따른다고 했잖아요.  
> 트레이닝하는 과정이 있고 그리고 샘플링하는 과정이 또 있다.  
> 근데 지금 샘플링하는 모양을 보면  
> VAE와 똑같이 생겼죠.  
> 뒤에 디코더 파트랑 똑같이 생겼거든요.  
>   
> 그 말은 뭐냐면  
> 훈련할 때에는 두 개의 Neural Network가 필요한데,  
> 
> 서비스를 하기 위해서  
> 샘플링을 하는 시간대에는 앞 부분을 다 버립니다.  
> 안 써요.  
> 훈련만 시켜 놓고 버리는 겁니다.  
> 실제로 쓰는 것은 디코더만이고,  
> 
...
> Convolutional 필터,  
> 데이터를 생성하기 위한 여러가지 장치들이 있을텐데  
> 그런 것들이 버려지는거에요.  
> inference 타임때는.  
>   
> 그래서 inference 타임 때  
> 데이터를 실제로 생성할 때는  
> VAE 같은 경우는 이 뒤의 파트만 사용합니다.  
...
> GAN 같은 경우는  
> $z$와 같은 latent space  
> $q_\phi$가 대상으로 되어야 하는 공간을  
> 설계를 할 수가 없는데  
> 
> VAE는 이런 독특한 구조 때문에  
> Anomaly detection에 사용할 수 있습니다.  
>   
> 훈련한 데이터에서 벗어난 데이터들을 탐지할 때  
> 이런 구조가 재밌습니다.  
> 
> $x$가 아니라, input이 $y$가 들어갔다고 생각해볼게요.  
> $y$가 들어가서 인코딩 한 다음에  
> 디코더를 거치면 $y'$이 나오겠죠.  
>   
> $y$랑 $y'$이 얼마나 distortion 됐는지를 재면  
> 훈련 과정에서 썼던 데이터인지 아닌지를 구별할 수 있습니다.  
> 
> 수학적인 이유도 있구요.  
>
> 우리가 $x$랑 $x'$의 거리를 0으로 만들도록 학습을 했는데  
> 그건 트레이닝 데이터 안에 있는 $x$에 해당하는 데이터인거고  
> 
> 완전히 새로운 $y$라는 데이터가 들어오게 되면  
> 인코더, 디코더 structure를 통해  
> $y'$이 나왔을 때 $y$랑 $y'$의 거리가 0이 된다는 보장은 없겠죠.  
> 우리가 그것을 훈련하지 않았기 때문에  
>   
> 그런 다름을 구별할 때 VAE structure를 많이 씁니다.  
>
> 그래서 몇몇 산업 AI에서 사용하는  
> (제가 과제로 진행하고 했었는데)     
...
> 온라인 상황에서,  
> 품질 검사 같은 것을 할 때,  
> defect가 존재하는 product들을 어떻게 감지를 할 것이냐?  
> 비주얼 센싱을 통해서 할 수도 있고  
> 일반적인 센서들을 통해서 할 수도 있는데  
> 타임 시리즈가 될 수도 있고  
>   
> defect를 가지고 있는 product들을 어떻게 탐지할 수 있는지?  
> 이런 것들이 Anomaly Detection이라는 태스크인데,  
> 보통 Auto Encoder structure들이 응용될 수 있다.  
> 이렇게 알아주시면 될 것 같습니다.  
>   
> 아까 말씀드렸었던 것처럼 inference 타임 때는 디코더만 사용을 한다.  
> $p_\theta$라는 것만 사용을 한다.  
>   
> 여러 가지 Gaussian Distribution, Gaussian Random Variable들이  
> 다른 종류의 이미지에 맵핑이 돼서  
> 이런 관점으로 KL divergence를 줄였기 때문에  
...
> (이건 $x$에 대한 것이었죠)  
> 
> $x$가 $q_\phi(z \mid x)$라는 인코더 structure를 통해서  
> $q_\phi(z)$라는 것으로 바뀌게 됩니다.  
> 이건 인코딩하는 과정이었고  
> 
> $q_\phi(z)$ 같은 경우는  
> Gaussian Random Variable로 모델링한다고 했었죠.  
> 모달리티가 쉽고 샘플링을 편하게 할 수 있고  
> 해석할 수 있는 그런 Distribution이라고 했습니다.  
>   
> 그리고 디코더에서 $p_\theta(x \mid z)$,  
> $z$가 input이고 $x$가 output이 되는  
> 디코더 structure를 통해서  
> 원래 데이터 $x$가 되기를 희망을 하는 겁니다.  
>   
> $p_\theta$로부터 샘플링된 $x'$들이랑 $x$랑  
> Reconstruction Loss를 통해서 같게 만들었다.  
> 이것이 VAE의 큰 내용이라고 생각합니다.  
>   
> 오늘 큰 강의의 줄기는 이 정도가 될 것 같은데  
> 인코더랑 디코더가 무엇인지,  
> 그리고 어떠한 철학적인 배경으로 어디에 응용될 수 있는지  
> 이런 것들을 기억해 주시면 좋을 것 같습니다.  
...
> 
> VAE가 latent space, latent information을  
> manipulation하는 데에 많은 장점들을 갖고 있는데,  
> 마지막 장의 예제를 보시면 좋을 것 같아요  
> 
> 왼쪽 그림은 high dimensional space,  
> latent space를 high-dimensional로 잡은 겁니다.  
> 256으로  
> 
> 각각의 숫자에 해당하는 것들을 레이블링 한 다음에  
> 그림으로 표현을 한 것인데  
> 
> 누가 시킨 게 아닌 것임에도 불구하고  
> (레이블을 주고 학습한 게 아니고,  
> conditional하게 학습한게 아닙니다)  
> 
> unconditional하게 학습했음에도 불구하고  
> 0에 대한 latent variable, 1에 대한 latent variable  
> 모양이 같으면 뭉치게 되고 클러스터링을 하게 되는데  
> 이것은 굉장히 재미있는 현상이거든요.  
>   
> 그래서 이런 현상 때문에  
> 위에서 직선을 그어서 latent variable들을 interpolation 한다고 가정을 하면  
>
> $z$라는 스페이스에서 interpolation 했기 때문에  
>
> 디코더를 통해 이미지를 생성하면  
> 
> 예를 들어서 첫 번째 시작점이 6이고 끝점이 4였다고 하면,  
> 6에서 4로 스무스하게 체인지가 되잖아요.  
> 이런 형상을 보여주는 것이 VAE의 굉장히 재미있는 특성입니다.  
>   
> latent space에서 정보들을 이렇게 알아서 클러스터링 해주는 것을  
> linearized effect라고 하는데  
> 
> 생성 모델들의 이런 것들을 통해 가지고...  
> 이게 어떤 의미냐면...  
>
> 이게 숫자로 보면 재미가 없을 수 있는데  
> 여러분들 많이 보셨잖아요.  
> 지브리 풍으로 바꾸는 거.  
> 그게 이거예요. 사실.  
> 
...
>
> 이 그림이 처음 나오고 거기까지 가는 게 딱 6년이 걸렸거든요.  
> 그러니까 이런 fundamental을 배워두셔야  
> 앞으로 나오는 기막힌 생성 모델의 철학 같은 것을  
> 이해하실 수 있을 것 같습니다.  
>   
> 오늘 말을 굉장히 빨리 하면서 진행을 했는데  
> 질문 있으시면 질문하시면 좋을 것 같습니다.  
>   
> (질문)  
>
> 마지막 그림에서 한시 방향으로는  
> distribution이 좀 많이 섞인 것 같은데  
> 그거는 좀 차원을 늘려서 모델링하면  
> 구분을 할 수 있는 레이블이 형성이 되나요?  
>   
> (답변)  
> 
> 차원을 단순히 늘리면    
> 표현하는 표현력이 떨어지게 되면서 어려워지겠죠.  
> 
> 더 정확한 말은 인코더와 디코더의 파라미터 숫자를 키워서 표현력을 키우고  
> 학습이 잘 되게 하는 Neural Network의 structure를 찾아서  
> 잘 학습하면 분리가 됩니다.  
>   
> (질문)  
>
...
> 그런데 확실한 거는 파라미터.  
> 
> 그러니까 인코더 디코더 Neural Network가  
> 파라미터 개수를 갖고 있고 학습력이 있거든요.  
> 
> 학습력이 안 좋은 상태로 이렇게 VAE를 학습하게 되면  
> 이 Separation, Separatability가 되게 떨어지게 됩니다.  
> 그러니까 그 말은 뭐냐면?  
> 지브리가 안 된다는 거예요.  
> 
> 그래서 여기 보면 잘 분리되는 애들의 모양들이 명확하거든요.  
> 그런데 잘 분리가 안 되는 애들은 그 사이사이가 좀 헷갈립니다.  
> 6 그리고 9, 5, 2 이런 스트럭처들이 헷갈리잖아요.  
> 그래서 이제 그런 거라고 보시면 됩니다.  
>   
> 그래서 현대 생성 모델들에서는 얘네들에 대해서  
> 어려운 부위와 어렵지 않은 부위를  
> 다르게 distribution 웨이트를 주는 방식으로,  
> 데이터의 학습력을 다르게 해서 학습을 하는,  
> 테크닉들이 많은데 저희 강의에서는 못 할 것 같습니다.  
> 그게 어드밴스드한 것들이에요.  
>   
> 또 질문 있으실까요?  
>   
> (질문)  
> 
> 네, reparameterization은 아까 설명하고 가신 건지  
> 제가 잘 못 들어가지고 이 부분도 중요한 건지?  
...
> 하나를 purified된 데이터셋이 있어서  
> 데이터가 굉장히 클린한 보통의 경우에요, 많은 세팅에서.  
> 그렇지 않은 경우도 있지만, 거의 그렇게 합니다.  
> 
> 그러니까 트레이닝 데이터 같은 경우는  
> expert들이 다 분류를 해서 Anomaly가 없다고 가정하는 거예요.  
> 다 분류를 해야겠죠. 이거는 정상 데이터라고 하는 등.  
>   
> 그래서 아까 온라인 환경이라고 얘기를 드렸는데  
> 온라인 환경에서는  
> defect가 있는지 없는지 클린한지 아닌지  
> 모르는 상황에서 쓰고 싶은 거잖아요.  
>
> 그래서 훈련 때에는  
> expert들이 분류를 해서,  
> 깔끔하다는 데이터로 VAE 훈련을 진행을 했을 때  
> 그럼 $x$랑 $x'$의 페어가 나오겠죠.  
> 그걸 훈련을 했기 때문에.  
>   
> 그런데 온라인 환경에서  
> $y$라는  
> defect가 있을 수도 있고 없을 수도 있는 데이터를  
> 인코더에 넣으면  
> $x$와 $x'$의 거리와  
> $y$와 $y'$의 거리가 완전히 달라집니다.  
>   
> $x$랑 $x'$ 같은 경우는  
> 트레이닝 데이터로 깔끔하게 만들었기 때문에 semantic도 분리가 됐고요.  
> $x$랑 $x'$ 사이의 거리를 줄이도록 학습을 했잖아요.  
> 그러니까 거리가 줄어든 상태일 겁니다.  
>   
> 그런데 온라인 환경에서  
> $y$라는 게 만약에 defect가 있다고 가정을 해봅시다.  
> 그럼 걔는 어떻게 될까요.  
> $y'$로 만들었을 때 거리가 어떻게 될까요.  
> 거리가 엄청나게 튀겠죠.  
>   
> 그리고  
> $y$가 만약에 defect가 없는 데이터가 들어오게 되면  
...
> 생성의 목적은 버리고 Anomaly Detection을 위해  
> 그 structure를 다 가지고 와서 인코더, 디코더를 같이 쓰는 거죠.  
> 원래는 그렇게 태어난 애가 아닌데.  
>   
> 그래서 VAE가 먼저 생성 모델로서 태어나고  
> 그 다음에 얘가 너무 아까우니까,  
> 인프라가 두 배 드는 이걸 어디다 갖다 쓸까 고민하다가  
> 오케이 그러면 그런 데에 한번 써볼까 하고  
> VAE의 어떤 어플리케이션으로서  
> Anomaly Detection으로 쓰면 좋겠다는 아이디어가 생겼고  
> 그렇게 사용하고 있는 것 같아요.  
