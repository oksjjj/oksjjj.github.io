...
> **강의 내용**  
> 
> G의 입장에서 봤을 때 D는 loss function입니다.  
> 왜냐하면, $G(x)$가 D의 input으로 들어갔잖아요.  
> 그래서 $\log D(\cdot)$가 G의 입장에서 봤을 때  
> loss function처럼 작동하는 거죠.  
>
> 앞에서 이렇게 얘기를 했습니다.  
> Neural Network의 output이  
> 뭔가의 objective function으로 래핑이 되는 것을 고려했는데,  
> 모양이 똑같이 생겼잖아요.  
>
> 그래서,  
...
> 근데 이게 조금 재밌는 거죠.  
>
> 앞에서 우리가 Image-to-Image Translation을 할 때  
> $L$을 Pixel-wise L2 Distance로 정의했었는데,  
> 이것은 움직이는 loss function인 거예요.  
>
> G 입장에서 보면,  
> D가 계속 학습되면서 바뀌고,  
> 그것을 옵티마이제이션 해야 되니까,  
...
> 어떤 식으로 훈련을 하느냐.  
> 
> 이제 앞으로 GAN 코드도 볼 겁니다.  
> 코드 implementation 하면서 확인을 할 텐데,  
> 
> Maximum D를 취한 다음에 Minimum G를 하면,  
> 정확하게 수학적으로는 잘 정리가 되어 있지만,  
> 코드 레벨에서 이것을 구현을 해서 학습시킨다는 건 사실 거의 불가능해요.  
>
> Maximizing D를 어떻게 할 것이냐.  
> 
> $G$는 처음에 Initialization이 어떻게 되어 있을까요?  
> Neural Network를 그냥 만들면,  
> 파라미터들이 쓰레기 값으로 차 있거든요.  
>
> 그러면 아무리 속여봤자 의미가 없는 거잖아요.  
> 서로 발전하면서 뭔가 표현이 되어야 되는데.  
>
> 그리고 이 수학 수식이 그것을 묘사를 하고 있는데,  
> 실질적으로 이런 Neural Network의 두 개의 중첩에 대해서,  
> 하나는 Gradient Ascent를 하고,  
> 하나는 Gradient Descent를 하는 것을 반복하게 됩니다.  
>
> 그래서 discriminator 입장에서는  
> Objective Function을 Maximizing 하는 거니까,  
> (Gradient Descent는 Optimization의 Loss Function 값을 줄이는 거였잖아요)  
>
> 그런데 discriminator 입장에서는 Maximization을 하니까  
> Gradient Ascent를 합니다.  
> 그러니까 Gradient가 원래 Minus 방향으로 줄이는 건데,  
> 역방향으로 하는 거죠.  
> 그렇게 하면 전체 Objective Function의 값이 커질 테니까.  
>
...
>
> 그래서 Generator를 잠그고  
> discriminator는 Gradient Descent 한 번 하고  
>   
> discriminator의 파라미터들을 잠그고  
> Generator가 Gradient Descent를 하고,  
>
> 그것을 무한 반복을 하는 거예요.  
>
> 아까 말씀드렸던 것처럼  
...
>
> 이 큰 블랙박스 모델이  
> Gradient Ascent와 Descent가 번갈아가면서 표현됐을 때,  
> 우리가 원하는 대로 수렴할 거라고 믿는 건  
> 바보인 거죠.  
>
> 그래서 바보들이었습니다.  
> 그래서 이게 잘 안 됩니다.  
> 이게 힘들었어요.  
>
> 자꾸 Optimizing해서 결과를 뽑아낸 거예요.  
> 이거 가지고 Product를 만드는 사람은 없습니다.  
>
> 요즘에 Image 생성 Model 같은 경우는  
> 엄청나게 Product화 되고 있어요.  
> (그 인프라를 유지하는 인력들은)  
>
...
> 그런 문제가 굉장히 심각합니다, 사실.  
>
> 여러분들 Objective Function을 막 설계를 하더라도  
> 결과를 쉽게 볼 수도 있는데,  
>
> 여러분들이 그냥 Colab에서 할 때는 아무런 문제 없지만,  
> 의미 있는 Semantic을 가지고,  
> 사람처럼 행동하는 그런 스케일로 커지면,  
>
> Objective Function을 다르게 설계하는  
> 이 작은 디테일의 차이로부터,  
> 엄청난 비용의 문제가 생길 수가 있더라고요.  
>
> 제가 직접 경험을 해보니까.  
>
> 그래서 실제로 스케일링업하는 세계에서는  
> 그런 것들이 되게 중요하다.  
>
> 그래서 GAN 모델은 이제  
...
>
> 그런 곳에서는 많이 연구되지만,  
> GAN 모델은 이런 트레이닝 stability 문제 때문에  
> 많이 사용되지는 않습니다.  
>
> 아까로 다시 돌아가서,  
...
> Gradient Ascent 한 번 하면,  
> 보통은 어떻게 되냐면,  
> 트레이닝 scheme이 다 망가져 버립니다.  
>
> 보통의 경우.  
> 최대한 수많은 노력을 해서,  
> discriminator와 generator의  
> 파라미터 숫자를 조절하고 해봐도,  
>
> 1대1의 업데이트 룰을 갖고 가면,  
> 트레이닝 전체가 다 collapse되요.  
>
> 그래서 보통은 어떻게 하냐면,  
>
> discriminator를 5번 업데이트 하고,  
> generator를 한 번 업데이트하고,  
> 이것을 번갈아가면서 합니다.  
>
> 그리고 여기서 또 중요한 게 뭐냐면,  
> 이 파라미터,  
> Representation Power를 잴 수 있거든요.  
>
> 특별한 metric을 써서,  
> Representation Power의 비율도 잘 정의를 해야 돼요.  
>
> learning하는 트레이닝 데이터에 따라,  
> learning하는 아키텍처에 따라,  
> 잘 조정을 해야 돼요.  
>
> 그래서 stable한 GAN learning을 보장을 하려면,  
> warmup이라고 해야 되나요?  
>
> 최적의 비율을 찾는 것을  
> 10번 정도 해야 돼요.  
...
> 그럼 그것이 어디서 유발이 됐냐?  
>
> 이 min-max 게임이 이론적인 솔루션이 없기 때문에,  
> 우리가 numerical하게 갖고 왔기 때문에,  
> 
> 간접적인 방향으로 그냥 물 떠다 놓고  
> 되기를 기도했기 때문에 그런 거고,  
> 뭔가 명확한 이론적 솔루션이 존재한 게 아니라.  
>
> 그래서 여러분들 그런 것에 대한 문제들도  
> 잘 생각을 해보셔야 될 것 같아요.  
>
> VAE나 GAN 같은 경우는  
> 그런 성격이 강해요.  
>
> 수학적인 contraint는 명확하게 주어지지 않고,  
> 뉴럴 네트워크로 그냥 모든 걸 다 풀어라.  
>
> 저저번 주에 variation equation을 배웠었는데,  
> variation equation도 사실,  
> upper bound 이러면서 막 뭐를 생략한다고 했잖아요.  
>
> 제가 그때 뭐라고 했었죠?  
> 원래는 명확하게 계산을 할 수 있어야 하는데,  
> (좀 어렵지만, 계산을 해야 정확하게 나오는데)  
> 그것을 날렸다고 했잖아요.  
>
> 그것도 이것과 비슷한 이유입니다.  
>
> 이론적으로 더 파고들고, 모델링했을 때의  
> complexity가 커지기 때문에  
> 다 날리고,  
> upper bound만 learning을 했기 때문에,  
>
> 걔네들도 instability가 똑같이 있고  
> 퀄리티도 떨어지고 하거든요.  
>
> 근데 이제 앞으로 배울  
> Normalizing Flow 라던지 Diffusion 모델은  
> 그런 것에 대한 mathmatical constraint를 더 명확하게,  
> 인간이 더 다루기 쉽게,  
> 컴퓨터 코드로 호환성 있게 설계를 해 놨습니다.  
>
> 오히려 그런 것들을 목적으로 만들었기 때문에.  
>
> 그래서 실제로 훈련을 하고 퀄리티를 재 보면,  
> 다른 모델들에 비해서  
> 좋은 퀄리티, 좋은 트레이닝 landscape를 볼 수가 있어요.  
>
> 그래서 그런 어떤 백그라운드의,  
> 엔지니어링적인 문제들도 존재한다.  
>
...
> **강의 내용**  
> 
> conditional GAN은 어떻게 하느냐,  
> 한번 생각해보자면,  
> 
> $x$라는 input 이미지(condition 이미지)가 있다고 할 때,  
> $G(x)$는 $G$가 만들어낸 fake 데이터입니다.  
> 
> conditional GAN 같은 경우는,  
> discriminator가,      
> condition 이미지와 fake 이미지 pair를  
> input으로 받습니다.  
>
> real 데이터에 대해서는,  
> condition 이미지와 target real 데이터 pair를,  
> discriminator input에 넣어서,  
> 학습하게 되고요.  
>
> 그리고 argmin하고 argmax 하는 것은 똑같습니다.  
> real, fake에 대해서,  
> 
> 앞에서 마찬가지로,  
> 1에 가깝게 되면 discriminator가 가짜라고 판별을 하는 거고,  
> 0에 가까우면 real이라고 판별을 하는 거죠.  
>
> conditioning만 추가로 input에 들어간다.  
> 이렇게 보시면 될 것 같아요.  
...
> 그래디언트 descent, ascent를 번갈아 수행가면서,  
> 굉장히 unstable 해진다고 얘기를 드렸었는데,  
> 
> 엔지니어들도 어쨌든 그것을 봉합을 해야 될 거 아니에요.  
> 되게끔 방법을 만들어야겠죠.  
>
> 그래서 Image Translation 같은 경우에는,  
> 뒤에 $\mathcal{L}_{L1}$이라고 써있는 것을 추가합니다.  
>
> 이것을 regularization term이라고 얘기하는데,  
> 
> 만들어 낸 데이터인 $G(x)$와 $y$라는 target 데이터가,  
> 서로 L1 distance 적으로 가깝게 하는,  
> 이런 추가적인 regularization term을 둬요.  
>
> 그래서,  
> generator와 discriminator가 diverge 되려고 할 때,  
> 이 term이 발동해서, diverge 되는 것을 최대한 막아주는 거죠.  
>
> 그래서 이것을 하면,  
> stable한 트레이닝을 보장 하고, 수렴도 빨리 됩니다.  
...
> 그런 예제들입니다.  
>  
> 이것은 18년도 쯤에 나왔던 논문들의 퀄리티거든요?  
> 근데 지금은 뭐 어마어마하겠죠.  
>
> colorization 뿐만 아니라 수많은 conditional한 생성을,  
> 하나의 파운데이션 모델에서 모두 처리할 수 있습니다.  
> 그런 획기적인 모델들이 많이 존재합니다.  
...
> **강의 내용**  
> 
> 앞에서 봤었던 Edge Detection 훈련된 모양들이,  
> 어떻게 결과물을 만들어낼까? 하는 생각을 해볼 수 있는데,  
>
> 그냥 이런 것을 하는 거예요.  
>
> 고양이 사진을 실컷 훈련을 해놓고,  
> 엉뚱하게 이 식빵처럼 생긴 것을 엣지로 주고,  
> 생성을 하라고 하는 거죠.  
>
> 근데 훈련은 고양이로 했지 않습니까?  
> 그러면 어떻게든, 내가 학습했었던 고양이의 모양에  
> 잘 배치를 해서 표현을 해요.  
>
> 그리고 재밌는 것은,  
> 저기에 눈이 있어야 될 것 같거든요?  
...
>
> 동그라미로 그려주면,  
> 사실 거기에 눈이 안 채워질 수도 있는데,  
> 근데 눈이 채워집니다.  
>
> 그게 고양이 얼굴의 기하학적인 것을 보면,  
> 눈이 항상 저기 있었기 때문에,  
> 그 확률이 높았었던 거죠.  
>
> 그래서 이렇게 표현이 되는 거고,  
> 다른 모양들에 대해서도 마찬가지입니다.  
>
> 엉뚱하지만 재밌게 잘 표현이 되는 겁니다.  
...
> 
> 왼쪽에 있는 층, 창문, 틀 같은  
> cascade한 input 이미지에 대해서 레이블링을 해주면  
> 오른쪽과 같이 생성해준다는 거죠.  
>
> 방이나 가구처럼 각진 것들은 생성이 더 잘 돼요.  
> 왜냐면 정형화된 틀이 있기 때문에.  
>
> 생성 모델에서도 훈련을 하다 보면,  
...
> 
> 자연에 있을 법한,  
> 각지고 정형화된 내추럴 오브젝트들에 대해  
> 이미지 생성을 해보면 퀄리티가 더 높습니다.  
>
> 정형화되지 않은 데이터라던지,  
> 데이터를 취득하기 힘든 내추럴 오브젝트의 모양이라던지,  
> 그런 것들을 생성하라고 하면,  
> 
> FID(Fresh Inception Distance)라고 하는,  
> 생성 모델 평가를 하는 메트릭이 있는데,  
> 점수가 압도적으로 떨어집니다.  
>
> 단순히 사람이 눈으로 보는 건 괜찮을지 몰라도,  
> 수치적으로 보면 굉장히 떨어집니다.  
>
> 이렇게 정형화된 것을 잘 만듭니다.  
>
...
>
> 사람의 얼굴 같은 것을 잘 만듭니다.  
> 눈, 코, 입 등 정형화된 위치들이 있잖아요.  
> 명확하게 기학적으로 뒤틀리면 안 됩니다.  
> 입이 코의 위치에 있으면 안 되잖아요.  
>
> 그래서 여러분들이 AI로 만든 사람을 보면,    
> 뭔가 이질적인 게 있잖아요.  
> 평균 이미지라는 얘기를 많이 해요.  
>
> 결국에는 그런 기하학적 패턴들을 확률적으로 정형화시켰기 때문에,  
> 우리가 느끼기에는 약간 이상하고 이질적인 것들이 평균치로 표현이 되는 건데요.  
> convolution filter에 화상처럼 남게 되면서.  
>
> 그런 큰 모델들의 convolution filter에 들어가면,  
> 기하학적 위치에 따라서  
> convolution filter들이 배치가 되는 걸 볼 수가 있어요.  
>
> 그런 것들이 다  
> 실제로 되게 재밌는 현상들이다.  
> 확률적인 현상들이다.  
...
> GAN은 뭐였죠?  
>
> 앞에서는 재미를 위해서 conditional 생성 모델을 주로 했는데,  
> 원래 GAN의 초창기는 unconditional 한 것이었습니다.  
> 
> $z$라는 Gaussian Distribution이 들어가면,  
> $x$라는 데이터가 나오고,  
> 이렇게 Gaussian Distribution에서 Data Distribution으로  
> 매핑을 해 주는 것이 generator입니다.  
>
> 그리고 min-max 게임을 통해서 학습합니다.  
...
> 
> 이것도 굉장히 오래된 논문인데,  
> 16년도나 17년도 논문에서 나왔었던 그림이라고 기억을 하고 있어요.  
>
> VAE 같은 경우는 blurry한 모양이 많이 나오죠.  
> 
> 이것은 reconstruction loss의 upper bound를 무시하고,  
> approximation된 걸 사용했기 때문에 그렇게 표현되는 거예요.  
> 정보들이 다 날라간 형태라고 보시면 될 것 같습니다.  
>
> 뉴럴 네트워크가 아무리 잘 구성이 되더라도,  
> 일정 수준 이상의 blur를 피할 수 있는 방법이 없어요.  
>
> 여기 보면,  
> 정형화된 패턴이 있는 얼굴 쪽은 잘 생성이 되는데,  
> 뒷 배경은 고주파수잖아요.  
>
> 사진을 찍으면 배경은 많이 달라지잖아요.  
> 어떤 곳은 빨간색 배경이 있고,    
> 그냥 막혀있는 방일 수도 있고,  
> 그래서 얼굴 쪽은 명확하게 다 표현이 되는데,  
> 뒷 배경은 고주파수로 다 날라갑니다.  
>
> GAN은 그런 것은 조금 해결이 되는 거죠.  
>
> 그런데 이것도 제가 말씀드렸던 것처럼,  
>
> 그래도 훈련 퀄리티가 괜찮고,  
> 앞에 warmup하는 시간을 다 지나가고,  
> stable한 트레이닝이 보장된 형태의,  
> GAN을 고려했을 때,  
> 그때의 샘플 퀄리티인 거예요.  
> 여기까지 보기가 되게 힘듭니다.  
>
> VAE는 샘플 퀄리티가 이렇게 조금 낮을 수는 있지만,  
> 훈련하기에 그렇게 어렵진 않고요.  
>
> 이렇게 두 개의 차이점이 있다고,  
> 이해하시면 될 것 같습니다.  
...
> **강의 내용**  
> 
> 오늘은 이 정도 얘기만 하면 될 것 같네요.  
> 이 그림에 대해서 얘기를 해보자면,  
> 
> 앞에서 log function으로 훈련을 한다고 되어 있었잖아요.  
> 바닐라 형태로 훈련을 하게 되면,  
> log를 미분할 때 심각한 문제가 생깁니다.  
>
> 빨간색 선으로 표시된 GAN discriminator는  
> log function에 물려있는 discriminator loss 항을  
> 표현을 하는 거에요.  
>
> 특별한 경우에 loss가 0이 되는  
> vanishing gradient 현상이 나타난다는 것을 표현하는 겁니다.  
>
> 그리고 GAN 말고, GAN의 확장 버전들이 있어요.  
>
> GAN을 수학 형식으로 generalized된 폼으로 표현한  
> Wasserstein GAN, F-GAN  
> 같은 것들이 있습니다.  
>
> KL divergence의 general한 형태인  
> Wasserstein distance라는 매트릭을 통한 GAN인데,  
> critic(discriminator) function이  
> gradient vanishing이 되지 않습니다.  
>
> GAN 같은 경우,  
> 트레이닝에서 instability한 프로퍼티를 컨트롤 하려고,  
> 수학하시는 분들, AI 리서처들이 오랫동안 지대한 노력을 많이 했습니다.  
>
> 그런데 이후,  
> Normalizing Flow, Diffusion과 같은,  
> 지금 생성 모델들이 나오면서,  
> 다 건너갔다고 보시면 될 것 같아요. 
>
> 노력을 아무리 해도 안 됐었거든요.   
>
> GAN은 많이 중요한데, 남아있는 유산들이 많이 있어요.  
> (현재 Diffusion 모델 같은 것을 훈련할 때도)  
>
> 유산은 많이 남겼지만,  
> 현대에서, 프로덕트 관점,  
> (연구용도 마찬가지입니다)  
...
>
> 그래도 한 시대의 패러다임이었었고,  
> 중요한 개념들을 많이 소개를 했었기 때문에,  
> 강의에서 소개를 드렸구요.  
>
> 오늘 메인 콘텐츠는 여기서 마무리하도록 하겠습니다.  
>
> 일단 질문을 받도록 하겠습니다.  
> 질문 있으실까요?  
>
