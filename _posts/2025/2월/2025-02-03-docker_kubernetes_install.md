...
### 1. VM 생성을 위한 리눅스 이미지를 다운 받습니다.  
* ubuntu 24.04 (Server용 LTS)  
  (맥북 애플 실리콘 버전은 arm 아키텍쳐로 다운로드 받습니다)  

[ubuntu 24.04 (x86) 다운로드](https://ubuntu.com/download/server){:target="_blank"}

[ubuntu 24.04 (arm) 다운로드](https://ubuntu.com/download/server/arm){:target="_blank"}

### 2. 아래와 같이 VM을 3개 생성하고 ubuntu를 설치합니다.  
...
### 3. VM 설치가 완료되면 모두 poweroff 하고, 1_os_install 이라는 명칭으로 스냅샷을 저장합니다.  
...
### 1. VM을 power on 하고 나서, root 패스워드를 설정 후 root 계정으로 전환합니다.
...
#### A. master, worker 노드 모두 chrony를 설치하고 설정 파일을 엽니다.  
```bash
apt install chrony
vi /etc/chrony/chrony.conf
```

#### B. chrony.conf 파일 설정 방법
...
### 8. 모두 poweroff 하고, 2_docker_install  이라는 명칭으로 스냅샷을 저장합니다. 
...
### 1. VM을 power on 하고 나서, root 계정으로 전환합니다.
...
#### A-1. x86 아키텍쳐용 go 언어 설치 방법 (주의: 애플 실리콘은 아래 명령어 실행 불가)
...
#### A-2. arm 아키텍쳐용 go 언어 설치 방법 (애플 실리콘은 아래 명령어로 설치)
...
#### B. x86, arm 모두 아래 명령어를 통해서 go 언어 설치 완료
...
#### A. kube-master 노드에서 아래 명령어를 실행합니다
...
#### B. 출력된 부분 뒤에 다음 내용을 붙여 넣어서 kube-worker1, kube-worker2에서 실행합니다  
```text
...
#### D. kube-master에서 아래 명령어를 실행해서, kube-master 및 kube-worker1,2가 NotReady 상태로 출력되는지 확인합니다  
...
##### a. kube-master에서 아래 명령어를 실행하고, 모든 pod가 running 및 ready 상태가 될 때까지 기다랍니다  
...
##### b. kube-master에서 아래 명령어를 실행해서 모든 노드가 Ready 상태로 출력되는지 확인합니다  
...
### 1. kube-master에서 아래 명령어를 실행합니다  

```bash
vim /etc/systemd/journald.conf
```

### 2. 아래 2개 단어가 있는 라인 2개를 주석 해제하고 (# 제거), 다음과 같이 수정합니다 (debug -> notice)  
...
### 3. kube-master에서 아래 명령어를 실행합니다  

```bash
systemctl restart systemd-journald
```

### 4. 모두 poweroff 하고, 3_kube_setup  이라는 명칭으로 스냅샷을 저장합니다. 
...
### 2. kube-master에서 kubectl get nodes 수행 시 아래와 같은 에러 메시지가 출력될 경우  
